...
public class PersonServiceImpl implements PersonService {
	...
	@Override
	public ErrorDto error(PersonEntity personEntity) {
		ErrorDto errorDto = new ErrorDto();
		java.sql.Date dob = personEntity.getBiodataEnt().getTglEnt();
		LocalDate today = LocalDate.now();
		LocalDate birthDate = dob.toLocalDate();
		Period p = Period.between(birthDate, today);
		if (((p.getYears() >= 30)) && (personEntity.getNikEnt().length() == 16)) {
			errorDto.setStatus("true");
			errorDto.setMessage("data berhasil masuk");
			return errorDto;
		} else if (((p.getYears() <= 30)) && (personEntity.getNikEnt().length() == 16)) {
			errorDto.setStatus("false");
			errorDto.setMessage("data gagal masuk, umur kurang dari 30");
			perRep.deleteById(personEntity.getNikEnt());
